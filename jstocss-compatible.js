...
/*
	JavaScript Object to CSS Transpiler. This is a version of JStoCSS compatible with basically every browser and incorporates elements of JS Versions before ES6.
...
function jscss_transpile(object) {
  ...
  var transpiled = ""; // Variable to store the transpiled CSS

  if (Object.keys(object).length > 0 && _typeof(object) === 'object' && !Array.isArray(object)) {
    ...
    for (var element in object) {
      if (object.hasOwnProperty(element)) {
        ...
        if (Array.isArray(object[element])) {
          ...
          for (var i in object[element]) {
            transpiled += object[element][i].toString().endsWith(';') ? "\t".concat(object[element][i], "\n") : "\t".concat(object[element][i], ";\n");
          }
        } else if (typeof object[element] === 'string') {
          transpiled += "\t".concat(object[element], "\n");
        } else if (_typeof(object[element]) === 'object') {
          // Iterating over the properties.
          for (var props in object[element]) {
            if (object[element].hasOwnProperty(props)) {
              transpiled += "\t".concat(props, " : ");
              transpiled += object[element][props].toString().endsWith(';') ? "".concat(object[element][props], "\n") : "".concat(object[element][props], ";\n");
            }
          }
  ...
  if (source) {
    ...
    if (typeof source === 'string') {
      ...
      if (Array.isArray(source)) {
        // Checking if the source is an array, as array has type object.
        throw new Error("Only pure JavaScript objects are permitted as sources for transpilation.");
      } else {
        // If the source is a pure object.
